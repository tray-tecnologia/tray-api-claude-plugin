...
- **`search_docs` (CLI e tool MCP `tray.search_docs`) voltou a indexar a documentação** ([#9](https://github.com/tray-tecnologia/tray-api-ai-plugin/issues/9)). A doc é servida como HTML renderizado, mas o índice era construído com `splitMarkdown` diretamente sobre o HTML — como o splitter só reconhece heading em Markdown (`# Título`), nenhuma seção era extraída e toda busca retornava `totalResults: 0`. O `parsed.md` do cache era cópia byte a byte do `raw.html`, sintoma do mesmo problema. Adicionado `scripts/lib/html-to-markdown.mjs`, a etapa de conversão que faltava entre o fetch e o splitter: preserva blocos de código com linguagem, converte as tabelas de campos em linhas Markdown, decodifica entidades acentuadas e descarta o menu lateral (que duplicaria todos os títulos no índice). Indexa ~820 seções da doc atual
- **Âncoras dos resultados apontam para a seção correta.** O anchor era derivado do título (`slugify`), o que divergia do `id` real em 46% das seções — a doc repete títulos como "Método GET" dezenas de vezes e desambigua com sufixo (`-2`, `-3`). O anchor do HTML agora é propagado até o resultado; o slug do título continua como fallback para entrada em Markdown
- **Cache do índice é invalidado por versão de pipeline.** `loadOrFetch` reaproveitava o índice sempre que o hash da doc não mudava, e `isFresh` só olhava o TTL. Um índice gerado por parser antigo (inclusive vazio) ficava preso até a doc de origem mudar, mesmo com `--refresh`/`--no-cache`. `metadata.indexVersion` passou a ser comparado com `INDEX_VERSION` nos dois caminhos, de forma que correções no parser cheguem sem limpeza manual de `~/.cache/tray-plugin/dev-docs/`
- **`--topic=listagem-carrinho` retornava vazio**: o mapa apontava para o H1 `Novo API de Listagem de Carrinho`, e o título na doc é `API de Listagem de Carrinho`
