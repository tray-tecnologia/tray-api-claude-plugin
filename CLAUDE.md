# Tray API Plugin — Contexto para Claude
...
Se executar requisições à API Tray (curl/node/wget/fetch) e receber:

- **HTTP 401** → `access_token` expirado ou inválido; renove via `GET https://{api_address}/auth?refresh_token={token}`.
- **HTTP 429** → rate limit; aplique backoff exponencial e/ou agrupe em lotes.
- **HTTP 400** com erro de validação → execute `skills/<recurso>/scripts/validate.mjs` com o payload antes de tentar de novo.
- **HTTP 404** em endpoint Tray → confirme o `api_address` (varia por loja, retornado no callback OAuth).
