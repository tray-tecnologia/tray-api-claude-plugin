...
#   TRAY_PRICE_LIST_VALUE_ID e confirmação explícita CONFIRM_DELETE=yes.
set -euo pipefail

: "${TRAY_API_BASE:?defina TRAY_API_BASE=https://api.sualoja.commercesuite.com.br}"
: "${TRAY_ACCESS_TOKEN:?defina TRAY_ACCESS_TOKEN}"
: "${TRAY_PRICE_LIST_ID:?defina TRAY_PRICE_LIST_ID=<id de uma lista de preço>}"
: "${TRAY_PRICE_LIST_VALUE_ID:?defina TRAY_PRICE_LIST_VALUE_ID=<id de um valor de teste>}"

if [ "${CONFIRM_DELETE:-no}" != "yes" ]; then
  echo "Operação destrutiva. Defina CONFIRM_DELETE=yes para excluir o valor ${TRAY_PRICE_LIST_VALUE_ID} da lista ${TRAY_PRICE_LIST_ID}." >&2
  exit 1
fi
